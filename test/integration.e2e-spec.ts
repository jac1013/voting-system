...
import {
  ResultInteractor,
  ResultInteractorImpl,
} from '../src/domain/use-case/result-interactor';
import { connectDatabase } from '../src/database/typeorm-config';
...
describe('Integration Test', () => {
  ...
  it('Ballot.vote() use case', async () => {
    ...
    // for (
    //   let i = 0;
    //   i < parseInt(process.env.NUMBER_OF_GENERATED_VOTES_FOR_INTEGRATION_TEST);
    //   i++
    // ) {
    ...
    // );
    //
    // const results = await resultInteractor.compute(election.id);
    // console.log(results);
    //
